...
class HybridStrategy(Strategy):
    ...
    def _aggregate_caps(self) -> StrategyCaps:
        """Merge capabilities from all child strategies.

        Rules:
        - requires_runtime: True if any child needs it
        - supports_fine_tuning: True if any child supports it
        - needs_sft_dataset: True if any child needs it
        - kl_regularization: Strongest level if any child uses KL
        - max_tokens_per_batch: Minimum token budget (most restrictive)
        - prefers_autocast: True unless any child disables it
        """
        ...
        needs_sft_dataset = any(cap.needs_sft_dataset for cap in child_caps)

        # Determine strongest KL regularization level
        kl_levels = {cap.kl_regularization for cap in child_caps}
        if "sequence" in kl_levels:
            kl_regularization = "sequence"
        elif "token" in kl_levels:
            kl_regularization = "token"
        else:
            kl_regularization = "none"

        # Use most restrictive token budget
        ...
        # Autocast preferred unless any child opts out
        prefers_autocast = all(cap.prefers_autocast for cap in child_caps)

        return StrategyCaps(
            requires_runtime=requires_runtime,
            supports_fine_tuning=supports_fine_tuning,
            needs_sft_dataset=needs_sft_dataset,
            kl_regularization=kl_regularization,
            max_tokens_per_batch=max_tokens,
            prefers_autocast=prefers_autocast,
        )
